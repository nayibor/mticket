...
class ReceptionController extends AppController {
    ...
    function batch_list($paginate_link = null) {
        ...
        $number_created = $this->Ticket->find("all", array(
            'group' => array('Ticket.ticket_batch_id'),
            'conditions' => array('Ticket.site_id' => $this->Session->read('site_id')),
            'fields' => array('ticket_batch_id', 'count(Ticket.id) ticketcount'), 'recursive' => "-1"));
        ...
        $real_number_created = array();
        foreach ($number_created as $key => $val) {
          //  print_r($val);
         //   echo $val['Ticket']['ticket_batch_id']."--".$val[0]['ticketcount'].' /n';
            $real_number_created[$val['Ticket']['ticket_batch_id']] = $val[0]['ticketcount'];
         ///   echo "------------";
        };
       // print_r($real_number_created);
      //  exit;
        $this->set(compact('layout_title', 'batches', 'real_number_created'));
    }
